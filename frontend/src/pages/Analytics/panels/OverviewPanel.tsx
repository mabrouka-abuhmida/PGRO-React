...
export const OverviewPanel: React.FC<OverviewPanelProps> = ({ statistics }) => {
  const statusData = statistics ? Object.entries(statistics.status_breakdown).map(([status, count]) => ({
    name: status.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase()),
    value: count
  })) : [];

  const degreeData = statistics ? Object.entries(statistics.degree_breakdown).map(([degree, count]) => ({
    name: degree,
    value: count
  })) : [];
          ...
          <Card variant="elevated" className="stat-card">
            ...
            <div className="stat-percentage">
              {statistics.total_applications > 0
                ? ((statistics.with_topics / statistics.total_applications) * 100).toFixed(1)
                : 0}%
            </div>
          </Card>
          <Card variant="elevated" className="stat-card">
            <h3>Research Groups</h3>
            <div className="stat-value">{statistics.research_group_coverage.length}</div>
          </Card>
        </div>
      )}
